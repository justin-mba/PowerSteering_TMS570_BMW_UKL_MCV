...
# if (ECUM_INCLUDE_RTE == STD_ON)
      Call_Rte_Start(); /* Nexteer Edit: Perform trusted function call to allow initialization of mode ports in other applications */
# endif
#else
...
STATIC FUNC(void, ECUM_CODE) EcuM_ActShutdownSystem(void)
{
  ...
  //EcuM_GlobalRestore(); Nexteer change to correct anomaly 2097 - interrupt protection restore moved to
...
# endif /* VECTOR_POLYSPACE_TEST */
    ...
    Mcu_PerformReset();
    EcuM_GlobalRestore(); //Nexteer change to correct anomaly 2097 - add interrupt protection around shutdown preparation.
#else
    /* Stop the RTE */

 ...
 *
 *	Description:       SID: 0x2F - Nexteer/ISO14229 defined service
 *
 *	Inputs:		       pMsgContext
 *
 *	Outputs:
...
)
{
   /* user extension                                                          */
	...
	{
		/* BAC checks returned NOK, additional checks are required to allow entry to Nexteer Session, Garage Mode,
		 * and session changes from those modes. BAC software does not consider these and will report NOK */

		switch(SesCtrlTypeNew)
		{
			case DEFAULT_SESSION:
				if((SesCtrlTypeActive == NEXTEER_SESSION) || (SesCtrlTypeActive == GARAGE_SESSION))
				{
					TempStatus = DCM_E_OK;
				}
			break;
	...
	if (ECUHardResetInProgress_Cnt_M_lgc == FALSE)
	{
		...
		if ( (SID == DIAG_UDS85_SERVICE_CONTROL_DTC_SETTING) &&
			 ((CurrentSession == EXTENDED_DIAGNOSTIC_SESSION) ||
			  (CurrentSession == NEXTEER_SESSION)) )
		{
...
-------   -------  --------  -----------------------------------  			   	  			  	  	----------
12/14/10   1.0      BDO		    Initial version - Nexteer Diagnositc		   						CR3728
								Services. Read SW part no.
12/16/10   2.0      BDO		    Added Write, Routine and I/O Ctrl 			   						CR3639, 3644, 3731
12/16/10   3.0      BDO  		Changed session check to allow Nexteer
								only.
12/16/10   4.0      BDO 		Removed Rev. 3.0 change above to allow
								default session access to Nexteer services
12/16/10   5.0      BDO 		Changed session check to allow Nexteer only
...
11/03/12   61       JJW         SID 0x11 Ecu Reset processing in Nexteer Session anomaly correction.

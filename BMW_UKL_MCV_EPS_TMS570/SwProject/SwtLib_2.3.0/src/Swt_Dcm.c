...
#include "Appl_Swt.h"
//#include "diag.h"  /* NEXTEER SWT - added include to Swt_Dcm.h */
...
///* NEXTEER SWT */ - CDCSWT Dcm_StatusType Swt_DcmGetSessionStartPermission( uint8 Sgid )
   ...
   for( jobIndex = 0; jobIndex < SWT_NUM_RCOS; jobIndex++ )
	...
	if( 0 != pJobDesc )
	{
		if( 0 != (pJobDesc->Flags & SWT_RCFLAG_REQUIRES_SWT_SESSION) )
		{
			...
			Dcm_GetSesCtrlType(&SesCtrlType);	///* NEXTEER SWT */ - CDCSWT  Dcm_GetSesCtrlType() )
...
void Swt_DcmReadDataByIdentifierReadEcuSwtList( CONSTP2VAR(Dcm_MsgContextType,AUTOMATIC,SC_FAR) pMsgContext )
{
	...
	if( DCM_SES_LEV_SWT != Dcm_GetSesCtrlType(&SesCtrlType) )  ///* NEXTEER SWT */ - CDCSWT  Dcm_GetSesCtrlType() )
	{
		Dcm_SetNegResponse( pMsgContext, DCM_NRC_SUBFUNCTIONNOTSUPPORTEDINACTIVESESSION );
		return;
	}
...
#if (SWT_USE_SWT_SHORT == SWT_CFG_ON)
      if (FALSE == isShort)   
#endif //NEXTEER SWT PATCH - moved #endif here was on the wrong line in original file according to BMW
      {
//NEXTEER SWT PATCH BMW expert Bjoern confirmed that this should have been on line 644: #endif
...
#endif
    }//NEXTEER SWT PATCH the brace is OK, it was the #endif that was on the wrong line according to BMW, functionally the same.

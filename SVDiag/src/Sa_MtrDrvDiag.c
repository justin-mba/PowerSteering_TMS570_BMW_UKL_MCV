 ...
 *     Workspace:  C:/CMSynergy_Projects/CBD/SVDiag/autosar
 *     SW-C Type:  Sa_MtrDrvDiag
 *  Generated at:  Thu Apr 25 10:47:28 2013
 *
 ...
 *   Boolean Rte_IRead_MtrDrvDiag_Per1_VswitchClosed_Cnt_lgc(void)
 *
 * Output Interfaces:
 * ==================
 *   Implicit S/R API:
 ...
 *********************************************************************************************************************/
	...
	VswitchClosed_Cnt_T_lgc = Rte_IRead_MtrDrvDiag_Per1_VswitchClosed_Cnt_lgc();
	...
	if (TRUE == VswitchClosed_Cnt_T_lgc)
	{
		if (TRUE == MtrDrvInitComp_Cnt_T_lgc)
		{
			...
			else
			{
				...
				if ((FALSE == FF1Set_Cnt_T_lgc) && (FALSE == FF2Set_Cnt_T_lgc))
				{
					...
					(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_GateDrvFlt, 0x01U, NTC_STATUS_PASSED);
					(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_OnStateSingleFET, 0x01U, NTC_STATUS_PASSED);
					(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_OnStateMultiFET, 0x01U, NTC_STATUS_PASSED);
				}
				else 
				{
															...
															CONSTP2VAR(uint16, AUTOMATIC, SA_MTRDRVDIAG_CONST) GenGateDriveFltAccPtr_Cnt_T_u16)
{
	...
	*OnStateFltAccPtr_Cnt_T_u16 = DiagPStep_m(*OnStateFltAccPtr_Cnt_T_u16, k_OnStateDiag_Cnt_str);
	...
	if (TRUE==DiagFailed_m(*OnStateFltAccPtr_Cnt_T_u16, k_OnStateDiag_Cnt_str))
	{
		...
		if (TRUE == OnStateFltActive_Cnt_T_lgc)
		{
		...
		else
		{
			...
			else if (FaultedPhases_Cnt_T_u16 == 1U)
			{
				SVDiag_ResetWaitLoop_Cnt_M_lgc = TRUE;
				(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_OnStateSingleFET, 0x01U, NTC_STATUS_FAILED);
			...
			else
			{
				...
				*GenGateDriveFltAccPtr_Cnt_T_u16 = *GenGateDriveFltAccPtr_Cnt_T_u16 + 1U;
				if(*GenGateDriveFltAccPtr_Cnt_T_u16 >= k_GenGateDrvFltThrsh_Cnt_u16)
				{
					*GenGateDriveFltAccPtr_Cnt_T_u16 = k_GenGateDrvFltThrsh_Cnt_u16;
					(void)Rte_Call_NxtrDiagMgr_SetNTCStatus(NTC_Num_GateDrvFlt, 0x02U, NTC_STATUS_FAILED);
			    }
			}
		}
...
STATIC INLINE FUNC(boolean, SA_MTRDRVDIAG_CODE) MotorDriverInit(void)
{
	...
	if (ElapsedTime_mS_T_u16p0  > k_GateDrvInitDwellTime_mS_u16p0)
	{
		ResetGateDrive();
